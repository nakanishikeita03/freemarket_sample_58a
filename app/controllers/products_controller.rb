class ProductsController < ApplicationController
  ...
  def index
    ...
    @appliancesproducts = Product.includes(:images).where(status: 0).where(category: 799..871).order("created_at DESC").limit(10)
    @toysproducts = Product.includes(:images).where(status: 0).where(category: 612..669).order("created_at DESC").limit(10)
